...
func main() {
	...
	minDistance := 1000.0
	bestCandidateEncrypted := ""
	bestCandidateDecrypted := ""
	for scanner.Scan() {
		line := scanner.Text()
		key, decryptedMessage, distance, _ := crackSingleCharXOREncryption(line)
		fmt.Println("\nBest candidate:", key, distance, decryptedMessage)
		if distance < minDistance {
			minDistance = distance
			bestCandidateDecrypted = decryptedMessage
			bestCandidateEncrypted = line
		}
	}
	fmt.Println("========")
	fmt.Println("\nBest candidate:", minDistance, bestCandidateDecrypted, bestCandidateEncrypted)
}
